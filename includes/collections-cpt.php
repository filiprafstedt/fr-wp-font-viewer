...
function fr_collection_ot_features_box($post) {
    ...
    wp_nonce_field(
        'fr_save_collection_features',
        'fr_collection_features_nonce'
    );

    if (empty($global_features)) {
        echo '<p><em>No global features defined yet.</em></p>';
        return;
    }
    ...
    echo '<thead>
            <tr>
                ...
                <th style="width:160px;">Show in Preview</th>
            </tr>
          </thead><tbody>';
    ...
    foreach ($global_features as $feature) {
        ...
        $enabled = $collection_features[$tag]['enabled'] ?? true;
        $preview = $collection_features[$tag]['preview'] ?? true;

        echo '<tr>';
        echo '<td>';
        echo '<strong>' . esc_html($feature['name']) . '</strong><br>';
        echo '<code>' . esc_html($tag) . '</code>';
        echo '</td>';

        echo '<td>';
        echo '<input type="checkbox"
            name="fr_collection_features[' . esc_attr($tag) . '][enabled]"
            ' . checked($enabled, true, false) . '>';
        echo '</td>';

        echo '<td>';
        echo '<input type="checkbox"
            name="fr_collection_features[' . esc_attr($tag) . '][preview]"
            ' . checked($preview, true, false) . '>';
        echo '</td>';
...
add_action('save_post_wpfv_collection', function ($post_id) {
    ...
    if (
        !isset($_POST['fr_collection_features_nonce']) ||
        !wp_verify_nonce(
            $_POST['fr_collection_features_nonce'],
            'fr_save_collection_features'
        )
    ) {
        return;
    ...
    $input = $_POST['fr_collection_features'] ?? [];
    $clean = [];

    foreach ($input as $tag => $values) {
        $clean[sanitize_key($tag)] = [
            'enabled' => isset($values['enabled']),
            'preview' => isset($values['preview']),
        ];
    }
